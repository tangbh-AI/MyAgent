...
# ============================================================
# 薄壁圆筒模态分析脚本
# 几何：长度 L=1000mm，外径 D=500mm，壁厚 t=5mm
# 材料：E=210000MPa，ν=0.3，ρ=7.8e-9 ton/mm³
# 边界：一端固支（Z=0端固定）
# 分析：提取前10阶固有频率及振型
# ============================================================
...
D_outer = 500.0     # 外径 (mm)
t = 5.0             # 壁厚 (mm)
r_mid = (D_outer - t) / 2.0  # 中面半径 = 247.5 mm

# ---------- 材料参数 ----------
E_modulus = 210000.0   # 弹性模量 (MPa)
nu = 0.3               # 泊松比
density = 7.8e-9       # 密度 (ton/mm³) — 7800 kg/m³

# ---------- 1. 创建模型 ----------
model = mdb.Model(name='Model-1')

# ---------- 2. 创建草图（XY平面，圆截面） ----------
# **注意**: ConstrainedSketch 没有 gridPlane 参数
sketch = model.ConstrainedSketch(name='TubeProfile', sheetSize=2.0 * r_mid)
...
part.BaseShellExtrude(sketch=sketch, depth=L)  # 沿Z轴拉伸1000mm

# ---------- 4. 材料定义 ----------
material = model.Material(name='Steel')
material.Density(table=((density,),))
material.Elastic(table=((E_modulus, nu),))
...
model.FrequencyStep(
    ...
    maxIterations=60,
    shiftPoint=None,
    acousticRange=(0.0, 0.0),
    projectDamping=OFF,
    useBoundaryDamping=OFF
)

# 场输出请求：保留默认输出（位移U、应力S等）
model.fieldOutputRequests['F-Output-1'].setValues(
    variables=('S', 'U', 'RF', 'CF')
)

# ---------- 8. 边界条件：底端（Z=0）固支 ----------
# 中面半径为247.5mm，底端圆周上一点 (r_mid, 0, 0)
fixed_edge_point = (r_mid, 0.0, 0.0)
fixed_edges = instance.edges.findAt((fixed_edge_point,))
fixed_region = regionToolset.Region(edges=fixed_edges)
model.EncastreBC(
    name='FixedEnd',

...
material.Elastic(table=((210000.0, 0.3), ))   # E=210 GPa, ν=0.3
material.Density(table=((7.8e-9, ), ))        # ρ=7800 kg/m³ = 7.8e-9 ton/mm³

# ==============================
# 创建部件: 薄壁圆筒 (壳单元)
# ==============================
# 草图: XY平面内绘制圆截面 (中面半径)
sketch = model.ConstrainedSketch(name='CircleSketch', sheetSize=2.0 * D)
sketch.CircleByCenterPerimeter(center=(0.0, 0.0), point1=(r_mid, 0.0))

# 使用 BaseShellExtrude 拉伸成壳 (沿Z轴)
part = model.Part(name='Tube', dimensionality=THREE_D, type=DEFORMABLE_BODY)
part.BaseShellExtrude(sketch=sketch, depth=L)

# 截面属性 (壳厚度)
section = model.HomogeneousShellSection(name='ShellSection',
                                        material='Steel',
                                        thickness=t)
...
model.FrequencyStep(name='Step-1', previous='Initial',
                    numEigen=10, eigensolver=LANCZOS, maxIterations=60)
...
part.seedPart(size=20.0)   # 全局网格尺寸 20 mm
part.generateMesh()
